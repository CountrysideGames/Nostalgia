...
//CONTROLES TOUCH
		...
		if (Game.time != 0)
		{
			...
			if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Moved)
			{
				...
				if (touchPosition.y > Screen.height/4 && touchPosition.y < (Screen.height/4) * 3 && touchDeltaPosition.x > 0)
					Rewind ();
			}
#if !UNITY_EDITOR
...
//SE O ESTADO FOR 1 (REWIND), COMEÇA UM CONTADOR DE TEMPO, QUE PAUSA APÓS 1 SEGUNDO
	...
	void Rewind ()
	{
		...
		if (roll[1].localScale.x < 1.08f && canPress)
		{
			...
			roll[0].transform.localScale = new Vector2 (rollScale[0].x - 0.009f, rollScale[0].y - 0.009f);
